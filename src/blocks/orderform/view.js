...
(function () {
  ...
  const processingState = {
    ...
    // Utility method to check if any operation is in progress
  ...
  function setupQuantityButtons() {
    ...
    document.body.addEventListener("click", async function (e) {
      ...
      if (productButton.classList.contains("carmo-bulk-product-plus-one"))
        increment = 1;
      else if (productButton.classList.contains("carmo-bulk-product-plus-five"))
        increment = 5;
      else if (productButton.classList.contains("carmo-bulk-product-plus-ten"))
        increment = 10;
  ...
  function setupCartHandlers() {
    ...
    document.body.addEventListener("keypress", async function (e) {
      if (e.target.matches(".carmo-bulk-quantity-input") && e.key === "Enter") {
        ...
        const cartItemKeyInput = input
          .closest("tr")
          .querySelector(".carmo-bulk-cart-item-key");
  ...
  /**
   * Update all button states based on processing flags
   */
  function updateButtonStates() {
    // Add to cart buttons
    document
      .querySelectorAll(".carmo-bulk-quantity-button")
      .forEach((button) => {
        button.disabled = processingState.addToCart;
        toggleProcessingClass(button, processingState.addToCart);
      });

    // Remove from cart buttons
    document.querySelectorAll(".carmo-bulk-category-reset-button").forEach((button) => {
      button.disabled = processingState.removeFromCart;
      toggleProcessingClass(button, processingState.removeFromCart);
    });

    // Update cart buttons
    document
      .querySelectorAll(".carmo-bulk-category-apply-button")
      .forEach((button) => {
        button.disabled = processingState.updateCart;
        toggleProcessingClass(button, processingState.updateCart);
      });

  }

  /**
   * Toggle processing class and update text
   */
  function toggleProcessingClass(button, isProcessing) {
    if (isProcessing) {
  ...
  /**
   * Handle page visibility changes to reset states if needed
   */
  function handleVisibilityChanges() {
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState === "visible") {
        // User has returned to the page, reset processing states
        // in case they navigated away during processing
        if (isDebug) console.log("Entered Reset Processing States");
        processingState.resetAll();
      }
    });

    // Also handle page show events (back/forward navigation)
    window.addEventListener("pageshow", function (event) {
      if (event.persisted) {
        // Page was restored from bfcache
        if (isDebug) console.log("Page was restored");
        processingState.resetAll();
      }
    });
  }
  ...
  async function applyValueSequentially(
    inputs,
    currentIndex,
    valueToApply,
    categoryId
    ...
    let progressElement = document.querySelector(
      `.carmo-bulk-progress[data-category-id="${categoryId}"]`
    );
    if (!progressElement) {
    ...
    try {
      ...
      if (result && Object.keys(result).length > 0) {
        ...
        console.warn(
          `Failed to update product: ${currentInput.dataset.productId}`
        );
      }
  ...
  function handleCategoryApply() {
    const categoryApplyButtons = document.querySelectorAll(
      ".carmo-bulk-category-apply-button"
    );
    ...
    categoryApplyButtons.forEach((button) => {
      button.addEventListener("click", function (event) {
        ...
        let categoryInput = document.querySelector(
          `.carmo-bulk-category-quantity-input[data-category-id="${categoryId}"]`
        );
        ...
        const valueToApply =
          categoryInput && categoryInput.value ? categoryInput.value : "1";

        // Find all inputs for the category
        const categoryInputs = document.querySelectorAll(
          `.carmo-bulk-quantity-input[data-category-id="${categoryId}"]`
        );

        if (isDebug)
          console.log(
            `Applying value ${valueToApply} to ${categoryInputs.length} products in category ${categoryId}`
          );

        // Start sequential processing
        applyValueSequentially(
    ...
    return updateCart(productId, newValue, cartItemKey).then((response) => {
      ...
      if (response && response.key) {
        ...
        if (!cartItemKeyInput) {
          ...
          row
            .querySelector(".carmo-bulk-product-quantity")
            .appendChild(newCartItemKeyInput);
      ...
      jQuery.ajax({
        ...
        beforeSend: function (xhr) {
          xhr.setRequestHeader(
            ...
            document.getElementById("carmo-bulk-form").dataset.nonce
          );
        },
        ...
        complete: function () {
          ...
          processingState.removeFromCart = false;
          updateButtonStates();
        },
      });
    });
  ...
  function updateCart(productId, quantity, cartItemKey) {
    if (isDebug)
      console.log("Starting updateCart", { productId, quantity, cartItemKey });

    // If already processing, don't submit again
    if (processingState.addToCart) {
      console.log("Add to cart operation already in progress");
      return Promise.reject("Operation in progress");
    }

    // Set processing flag
    processingState.addToCart = true;
    updateButtonStates();
    ...
    return new Promise((resolve, reject) => {
      ...
      let ajaxConfig = {
        ...
        complete: function () {
          // Reset processing flag
          processingState.addToCart = false;
          updateButtonStates();
        },
      };
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        inputs.forEach(function (input) {
          ...
          const row = input.closest("tr");
          if (row) {
            const cartItemKeyInput = row.querySelector(
              ".carmo-bulk-cart-item-key"
            );
            if (cartItemKeyInput) {
              cartItemKeyInput.value = "";
            }
        ...
        inputs.forEach(function (input) {
          ...
          if (productsInCart[mapKey]) {
            ...
            let cartItemKeyInput = row.querySelector(
              ".carmo-bulk-cart-item-key"
            );
            if (!cartItemKeyInput) {
